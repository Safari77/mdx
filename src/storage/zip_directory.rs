...
impl directory::Directory for ZipDirectory {
    fn get_file_handle(
        ...
        std::sync::Arc<dyn directory::FileHandle>,
        directory::error::OpenReadError,
    > {
        let entry_info = self.get_entry_info(path).map_err(|e| {
            directory::error::OpenReadError::wrap_io_error(
    ...
    fn delete(
        &self,
        path: &std::path::Path,
    ) -> std::result::Result<(), directory::error::DeleteError> {
        Err(directory::error::DeleteError::IoError {
            io_error: std::sync::Arc::new(io::Error::new(
    ...
    fn exists(
        &self,
        path: &std::path::Path,
    ) -> std::result::Result<bool, directory::error::OpenReadError> {
        self.has_entry(path)
    ...
    fn open_write(
        &self,
        path: &std::path::Path,
    ) -> std::result::Result<directory::WritePtr, directory::error::OpenWriteError> {
    ...
    fn atomic_read(
        &self,
        path: &std::path::Path,
    ) -> std::result::Result<Vec<u8>, directory::error::OpenReadError> {
